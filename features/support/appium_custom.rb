...
class AppiumCustom
  def button_exists?(button)
    # Verificando a existência do botão #{el} usando o tipo de busca XPATH
    $driver.find_elements(:xpath, button).count > 0
    end

  def wait_for_button_to_exist(button)
    # Aguardou a existência do botão #{el} usando o tipo de busca XPATH
  ...
  def click_button(button)
    ...
    # Clicou no botão #{el} usando o tipo de busca XPATH
  end
  ...
  def wait_for(el)
    ...
    # Aguardou a exibição do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
  end
  ...
  def wait_for_element_to_exist(el)
    ...
    # Aguardou a existência do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
  end
  ...
  def click(el)
    ...
    # Clicou no botão ' + el['value'] + ' usando o tipo de busca ' + el['tipo_busca']
  end
  ...
  def click_index(el, index)
    ...
    # Clicou no indice #{index} do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
  end
  ...
  def click_subelement_index(element, el, index)
    ...
    # Clicou em um subelemento no indice #{index} do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
  end

  def element_exists?(el)
    # Verificando se existe o elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
    case el['tipo_busca']
    when 'XPATH'
      $driver.find_elements(:xpath, el['value']).count > 0
    when 'ID'
      $driver.find_elements(:id, el['value']).count > 0
    when 'CLASS'
      $driver.find_elements(:class, el['value']).count > 0
    end
  end
  ...
  def elements(el)
    case el['tipo_busca']
    when 'XPATH'
      $driver.find_elements(:xpath, el['value'])
    when 'ID'
      $driver.find_elements(:id, el['value'])
    when 'CLASS'
      $driver.find_elements(:class, el['value'])
    end
    # Buscou a lista de elementos ' + el['value'] + ' usando o tipo de busca ' + el['tipo_busca']
  end

  def element_is_enabled?(el)
    # Elemento habilitado #{['value']} usando o tipo de busca #{el['tipo_busca']}
    case el['tipo_busca']
    when 'XPATH'
      $driver.find_elements(:xpath, el['value']).enabled
    when 'ID'
      $driver.find_elements(:id, el['value']).enabled
    when 'CLASS'
      $driver.find_elements(:class, el['value']).enabled
    end
  end

  def get_text(el)
    # Está buscando o texto do elemento ' + el['value'] + ' usando o tipo de busca ' + el['tipo_busca']
    case el['tipo_busca']
    when 'XPATH'
      $driver.find_element(:xpath, el['value']).text
    when 'ID'
      $driver.find_element(:id, el['value']).text
    when 'CLASS'
      $driver.find_element(:class, el['value']).text
    end
  end

  def get_subelement_text_index(element, el, index)
    # Buscando texto em subelemento no indice #{index} do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
    case el['tipo_busca']
    when 'XPATH'
  ...
  def get_text_index(el, index)
    # Indice do texto #{index} do elemento #{el['value']} usando o tipo de busca #{el['tipo_busca']}
    case el['tipo_busca']
    when 'XPATH'
      $driver.find_elements(:xpath, el['value']).get(index).text
    when 'ID'
      $driver.find_elements(:id, el['value']).get(index).text
    when 'CLASS'
      $driver.find_elements(:class, el['value']).get(index).text
    end
  end
  ...
  def fill_in(el, text)
    ...
    # Preencheu o campo #{el} usando o tipo de busca #{el['tipo_busca']} com o valor #{text}
  end
  ...
  def send_keys(el, text)
    case el['tipo_busca']
    when 'XPATH'
      $wait.until { $driver.find_element(:xpath, el['value']).send_keys text }
    when 'ID'
      $wait.until { $driver.find_element(:id, el['value']).send_keys text }
    when 'CLASS'
      $wait.until { $driver.find_element(:class, el['value']).send_keys text }
    end
    # Achando o elemento #{el['value']} usando o send_keys
  end
